 ...
 */
package com.feilong.spring.web.handler.encryptor;
 ...
 * @since 1.12.0
 */
public class SymmetricJsonHexEncryptor implements Encryptor{
     ...
     */
    @Override
    public String encrypt(Serializable bean){
     ...
     */
    @Override
    public <T> T decrypt(String encryptPayResultType,Class<T> klass){
        SymmetricEncryption symmetricEncryption = build();
        String json = symmetricEncryption.decryptHex(encryptPayResultType, charsetName);

        //---------------------------------------------------------------
        if (LOGGER.isDebugEnabled()){
            LOGGER.debug("encryptPayResultType:[{}],json:[{}]", json);
        }
